...
Solid components execute their body **once**; they are not re-invoked when state changes. If you use
early returns like `if (!data) return null`, the component will never re-run that check. The
returned JSX is static; only the reactive expressions within it update. This is the biggest mental
model difference from React, where components re-render on every state change.
...
1. **Migration Safety**: This is the single most common mistake when migrating from React to Solid.

1. **Reactivity**: Solid's reactive system only tracks signals accessed within reactive contexts
   (JSX expressions, effects, memos). Code in the component body runs once and is never revisited.

1. **Debugging**: Bugs from early returns are subtle; the component appears to work on first render
   but never updates.

## Related Rules

- [3-1: Use Show for Conditionals](3-1-use-show-for-conditionals.md) - The Show component for
